...
linux-amd64:
	@env GOOS=linux GOARCH=amd64 go build -o deploy/binary/$(NAME)-linux-amd64 cmd/devopsci/main.go

## linux-arm64: Compile linux-amd64 package
linux-arm64:
	@env GOOS=linux GOARCH=arm64 go build -o deploy/binary/$(NAME)-linux-arm64 cmd/devopsci/main.go

.PHONY: build
## build: Compile the packages.
build:
	@go build -ldflags '$(LDFLAGS)' -o $(NAME) cmd/devopsci/main.go
