...
import (
	...
	"gitee.com/plutoccc/devops_app/internal/core/apps"
	"gitee.com/plutoccc/devops_app/internal/core/kuberes"
...
var (
	atomciServer = beego.AppConfig.String("start::url")
)
...
func (pm *PipelineManager) CreateBuildJob(creator string, projectID, publishID int64, envStageJSON *PipelineStageStruct, apps []*RunBuildAppReq, customeEnvVars []EnvItem) (int64, string, error) {
	...
	callBackURL := fmt.Sprintf("%s/start/api/v1/pipelines/%d/publishes/%d/stages/%d/steps/%s/callback", atomciServer, projectID, publishID, envStageJSON.StageID, "build")
...
func (pm *PipelineManager) CreateDeployJob(creator string, projectID, publishID int64, stageJSON *PipelineStageStruct, apps []*RunDeployAppReq) (int64, string, error) {
	...
	callBackURL := fmt.Sprintf("%s/start/api/v1/pipelines/%d/publishes/%d/stages/%d/steps/%s/callback", atomciServer, projectID, publishID, stageJSON.StageID, "deploy")
	callBackRequestBody := fmt.Sprintf("{\"publish_job_id\": %d}", publishJobID)
